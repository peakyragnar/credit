...
Quirk #10 — description column: the raw text places a dot-run column separator IMMEDIATELY after the CUSIP, so a naive "text up to the first dot-run" description window is always empty. Rule: skip the leading run, then read to the next ≥4-dot run. The captured string is issuer name + printed description concatenated (occasionally with bond-characteristic column bleed, e.g. city/state tokens — noise, not signal). Parser-upgrade protocol used here and generalized: after any parser change, `tools/check_d1_reparse.py` gates the new extract — every non-target column must be byte-identical to the committed version, else FAIL.

### Era-2 parser quirks (period backfill, 2026-07-22)
The pre-2025 ("era-2") Schedule D Part 1 runs through the same parser with `YEARS` config (year → pdf, sha, page span, banked total, tolerance). Era differences, each learned from a footing failure:
- **Quirk #11 — rate-used column.** Era-2 interleaves a 4-decimal "Rate Used to Obtain Fair Value" column between actual cost and fair value; it must be excised before positional cell mapping. Column order also swaps: era-1 = cost,par,FV,BACV; era-2 = cost,FV,par,BACV.
- **Quirk #12 — Bond-Characteristics false designation.** Era-2 rows carry a bare digit (2, 4…) in the Bond Char column right before the designation; a designation regex with an optional dot false-matches it and shifts every money cell right by two (cost→par, FV→BACV; caught because par almost footed while cost was half missing). Era-2 designation regex requires the digit-DOT form ('2.C', '6.').
- **Quirk #13 — three print forms of the rate.** Attached ('...31.6210'), detached ('.. 86.6800' — the orphan dot-run must be excised WITH the rate or it becomes a phantom blank FV), and comma-split ('14,685,163' + '.0000' — pypdf splits a comma-grouped rate into two tokens). Excision tries most-specific first.
- **Quirk #14 — split-cell ambiguity (bounded, unresolved).** The token pair [dot-run][bare number] is genuinely ambiguous: usually one split value cell (rule required — disabling it breaks footing by $B), occasionally a blank cell + detached value (shifts BACV into par on written-off legacy rows). Residual impact measured and logged: 2023 −$326,724 (4.3 ppm), category 102 only. Candidate fix if ever needed: column-width fitting (expected 8 money slots per row).
- **Diagnostic pattern that found all of these:** walk the in-stream category subtotals (printed cost/FV/par/BACV per category) against running parsed sums — the four-column diff signature identifies the failure mode (cost exact + BACV short = slot shift; all short = dropped rows).
